...
class SingleVideoAnticipationDataset(Dataset):
    ...
    def __init__(self, fused_csv_path, label_csv_path,
        ...
        if "frame_idx" in fused_df.columns:
            fused_df = fused_df.rename(columns={"frame_idx": "frame_index"})
        elif "frame_index" not in fused_df.columns:
            raise KeyError("fused CSV must contain 'frame_idx' or 'frame_index' column")
        fused_df["frame_index"] = fused_df["frame_index"].astype(int)
        fused_df["frame_idx"]   = fused_df["frame_index"]
        ...
        # Support both feat_0 and fused_0 column naming conventions
        if f"feat_0" in fused_df.columns:
            self.feat_cols = [f"feat_{i}" for i in range(self.feat_dim)]
        elif f"fused_0" in fused_df.columns:
            self.feat_cols = [f"fused_{i}" for i in range(self.feat_dim)]
        else:
            raise KeyError("Cannot find feature columns (expected feat_0 or fused_0)")
        self.fps        = float(fps)
