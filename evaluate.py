...
# ── Load model ──
    ...
    if "model_state" in ckpt:
        ...
        # Infer num_classes directly from checkpoint weights
        state_dict  = ckpt
        num_classes = {
            "verb":   ckpt["verb_head.weight"].shape[0],
            "noun":   ckpt["noun_head.weight"].shape[0],
            "action": ckpt["action_head.weight"].shape[0],
        }
        feat_dim   = FEAT_DIM
        k_fut      = K_FUT
        horizons_s = HORIZONS_S
        print(f"[INFO] Inferred num_classes from checkpoint: {num_classes}")
